...
@interface ContactExplanationPopupView()
...
@implementation ContactExplanationPopupView
@synthesize delegate;
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    self.imageView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"contactFriendsModalBackground"]];
    ...
    UIImageView *drinkIcon = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"hotspotDrinkIcons"]];
//    drinkIcon.size = CGSizeMake(30, 150);
    ...
    headerTitle.y = 290;
    headerTitle.text = @"TEXT FRIENDS WITH HOTSPOT";
    [self.imageView addSubview:headerTitle];
    
    UILabel *textLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 300, self.width - 110, 80)];
    textLabel.centerX = self.width/2;
    textLabel.font = [ThemeManager lightFontOfSize:12];
    textLabel.textAlignment = NSTextAlignmentCenter;
    textLabel.numberOfLines = 4;
    textLabel.text = @"Select friends to meet up and we’ll send them your message. If they’re new to Hotspot, both your drinks are free.";
...
//    textLabelLineTwo.attributedText = attributedText;
    ...
    [launchInviteButton setTitle:@"SYNC CONTACTS" forState:UIControlStateNormal];
    ...
    self.doneButton.titleLabel.font = [ThemeManager regularFontOfSize:12];
...
- (void)dismissAndOpenInviteView
{
    [UIView animateWithDuration:0.5 animations:^{
        ...
        [self.delegate requestContactPermissions];
...
- (void)dismiss
{
    [UIView animateWithDuration:0.5 animations:^{
        ...
        [self.delegate skipButtonTouchedFromContactModal];
        [self removeFromSuperview];
    }];
}
